...
function getPolynom(...args) {
  return function polyndrom(value) {
    const digits = args.length - 1;
    let res = 0;
    args.forEach((digit, i) => {
      res += digit * value ** (digits - i);
    });
    return res;
  };
}
...
function retry(func, attempts) {
  const funcRetry = () => {
    ...
    try {
      ...
      if (attempts > 0) return funcRetry(func, attempts - 1);
    }
    return result;
  };
  return funcRetry;
...
function logger(func, logFunc) {
  return (...args) => {
    const ar = args.map((item) => JSON.stringify(item)).join(',');
    logFunc(`${func && func.name}(${ar}) starts`);
    const result = func(...args);
    logFunc(`${func && func.name}(${[ar]}) ends`);
    return result;
  };
}
